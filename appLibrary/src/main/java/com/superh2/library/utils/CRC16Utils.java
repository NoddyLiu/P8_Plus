...
public class CRC16Utils
{
    ...
    public static String getCrc(int res) {
        ...
        return substring.concat(" ").concat(substring1);
    }

    public static String getCrc1(byte[] data) {
        int res=calcCrc16(data, 0, data.length);
        String format = String.format("%04x", res);
        String substring = format.substring(0, 2);
        String substring1 = format.substring(2, 4);
        Log.i("BLUEDATA", "crc ---- : " + substring + "  " + substring1);
        byte b1=new Byte(substring);
        byte b2=new Byte(substring1);
        return substring.concat(" ").concat(substring1);
    }
    ...
    public static Byte[] getCrc(byte[] data) {
        ...
        String substring1 = format.substring(2, 4);
        Log.i("BLUEDATA", "crc ---- : " + substring + "  " + substring1);
        byte b1 =  Integer.decode("0x"+substring).byteValue();
        byte b2 = Integer.decode("0x"+substring1).byteValue();
        Byte[] bt=new Byte[]{b2,b1};
        return bt;
    }
    ...
    public static String conver16HexStr(byte[] b) {
        ...
        for (int i = 0; i < b.length; i++) {
            if ((b[i] & 0xff) < 0x10)
                result.append("0");
            result.append(Long.toString(b[i] & 0xff, 16).concat(" "));
        }
